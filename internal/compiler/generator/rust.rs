...
fn compile_builtin_function_call(
    ...
    match function {
        ...
        BuiltinFunction::ItemAbsolutePosition | BuiltinFunction::ItemAbsoluteNativePosition => {
            if let [Expression::PropertyReference(pr)] = arguments {
                let item_rc = access_item_rc(pr, ctx);
                let map_fn = if matches!(function, BuiltinFunction::ItemAbsoluteNativePosition) {
                    quote!(map_to_native_window)
                } else {
                    quote!(map_to_window)
                };
                quote!(
                    sp::logical_position_to_api((*#item_rc).#map_fn(::core::default::Default::default()))
                )
            } else {
        ...
        BuiltinFunction::MapPointToNativeWindow => {
            if let [Expression::PropertyReference(pr), point] = arguments {
                let item_rc = access_item_rc(pr, ctx);
                let point = compile_expression_to_value(point, ctx);
                quote!({
                    let p: sp::LogicalPosition = #point;
                    sp::logical_position_to_api((*#item_rc).map_to_native_window(
                        sp::euclid::point2(p.x as sp::Coord, p.y as sp::Coord)
                    ))
                })
            } else {
