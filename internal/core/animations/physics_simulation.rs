...
pub trait Simulation<Quantity> {
    fn step(&mut self) -> (Quantity, bool);
    fn curr_value(&self) -> Quantity;
}

pub trait Parameter<Quantity> {
    type Output;
    fn simulation(self, start_value: Quantity, limit_value: Quantity) -> Self::Output;
}
...
impl ConstantDecelerationParameters {
    pub fn new(initial_velocity: Velocity, deceleration: Acceleration) -> Self {
        Self { initial_velocity, deceleration }
    }
}

#[allow(dead_code)]
impl Parameter<Length> for ConstantDecelerationParameters {
    type Output = ConstantDeceleration;
    fn simulation(self, start_value: Length, limit_value: Length) -> Self::Output {
...
impl ConstantDeceleration {
    ...
    fn new_internal(
        ...
        let direction = if start_value == limit_value {
            ...
            data.deceleration = data.deceleration.abs();
            assert!(initial_velocity.is_sign_positive()); // Makes no sense yet that the velocity goes into the other direction
    ...
    fn step_internal(&mut self, new_tick: Instant) -> (Length, bool) {
        ...
        self.curr_val += (self.velocity + new_velocity) / 2. * duration; // Trapezoidal integration
        ...
        match self.direction {
            Direction::Increasing => {
                ...
                } else if self.velocity.is_sign_negative() || self.velocity.is_zero() {
                    return (self.curr_val, true);
                }
            }
            Direction::Decreasing => {
                if self.curr_val <= self.limit_value {
                    ...
                    self.velocity.set_zero();
                    return (self.curr_val, true);
                } else if self.velocity.is_sign_positive() || self.velocity.is_zero() {
                    return (self.curr_val, true);
                }
            }
        }
...
#[allow(dead_code)]
impl Simulation<Length> for ConstantDeceleration {
    fn curr_value(&self) -> Length {
        self.curr_val
    }

    fn step(&mut self) -> (Length, bool) {
